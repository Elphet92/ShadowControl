...
(Для запуска .ps1 необходимо разрешить выполнение скриптов в вашем Powershell)
3. Введите IP-адрес или имя компьютера (автодополнение из AD подскажет доступные имена).
4. Укажите ID сессии (обычно 1).
5. Выберите нужные параметры подключения (/control, /multimon, /span, /prompt, проверка ping).
6. Нажмите «Подключиться» или клавишу Enter.
